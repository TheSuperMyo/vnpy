...
class TSMyoRSIStrategy(CtaTemplate):
    """"""
    ...
    parameters = ['rsi_fliter_slow','rsi_fliter_fast','rsi_len','setup_bar','trailing_stop','fixed_size']
    variables = ['bar_counter','rsi_value_slow','rsi_value_fast','stop_long','stop_short']

    def __init__(self, cta_engine, strategy_name, vt_symbol, setting):
        """"""
        super(TSMyoRSIStrategy, self).__init__(
            cta_engine, strategy_name, vt_symbol, setting
        )
        self.bg = BarGenerator(self.on_bar,5,self.on_5min_bar)
        self.am_fast = TSMArrayManager()
        self.am_slow = TSMArrayManager()
        # 策略自身订单管理
    ...
    def on_bar(self, bar: BarData):
        ...
        am_fast = self.am_fast
        am_fast.update_bar(bar)
        if not am_fast.inited:
            return
        ...
        if ( last_bar.datetime.date() != bar.datetime.date() ):
            self.bar_counter = 1
            self.rsi_value_fast = 0
            self.rsi_value_slow = 0

        if self.bar_counter < self.setup_bar:
            return

        self.rsi_value_fast = am_fast.rsi(self.rsi_len, False)
        
        if self.pos == 0 and bar.datetime.time() < self.exit_time:
            if self.rsi_value_fast > self.rsi_fliter_fast and self.rsi_value_slow > self.rsi_fliter_slow:
                # 入场开多
            ...
            if self.rsi_value_fast < (100-self.rsi_fliter_fast) and self.rsi_value_slow < (100-self.rsi_fliter_slow):
                # 入场开空
                if self.active_orderids:
                    self.write_log("撤单不干净，无法挂单")
                    return
    ...
    def on_5min_bar(self, bar: BarData):
        am_slow = self.am_slow
        am_slow.update_bar(bar)
        if not am_slow.inited:
            return
        self.rsi_fliter_slow = am_slow.rsi(self.rsi_len, False)
