...
class TSMyoGPTestStrategy(CtaTemplate):
    """"""
    ...
    parameters = ['trailing_stop','fixed_size']
    variables = ['bar_counter','stop_long','stop_short']

    def __init__(self, cta_engine, strategy_name, vt_symbol, setting):
        """"""
        super(TSMyoGPTestStrategy, self).__init__(
    ...
    def sign_func(self,ll,tt):
        return np.cos(tt) + 2*np.sin( np.cos(ll+tt) / np.arccos(np.sin(ll)**tt) )
    ...
    def on_5min_bar(self, bar: BarData):
        ...
        sign = self.sign_func(bar.low_price,self.bar_counter)
