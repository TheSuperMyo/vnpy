...
class TSMyoMADONStrategy(CtaTemplate):
    """"""
    ...
    LLT_len_slow =  20
    LLT_len_fast =  10
    donchian_len_fast = 10
    donchian_len_slow = 20
    
    trailing_stop = 0.8 # 跟踪止损
    fixed_size = 1 # 固定手数
    ...
    parameters = ['LLT_len_slow','LLT_len_fast','donchian_len_fast','donchian_len_slow','trailing_stop','fixed_size']
    variables = ['bar_counter','stop_long','stop_short']
    ...
    def on_bar(self, bar: BarData):
        ...
        # 确保不受昨日数据影响（防大幅跳价）
        if self.bar_counter < max(self.LLT_len_slow, self.donchian_len_slow):
            return

        LLT_value_fast = am.LLT(self.LLT_len_fast, True)
        LLT_value_slow = am.LLT(self.LLT_len_slow, True)
        #print(bar.close_price,self.LLT_value_fast[-2],self.LLT_value_slow[-2],self.LLT_value_fast[-1],self.LLT_value_slow[-1])
        donchian_value_fast_up, donchian_value_fast_down = am.donchian(self.donchian_len_fast, False)
        donchian_value_slow_up, donchian_value_slow_down = am.donchian(self.donchian_len_slow, False)

        # 金叉
        if LLT_value_fast[-2] < LLT_value_slow[-2] and LLT_value_fast[-1] > LLT_value_slow[-1]:
            # 大致记录金叉点
            self.cross_up = (LLT_value_slow[-1] + LLT_value_slow[-2])/2
            # 如果突破
            if self.pos == 0 and bar.datetime.time() < self.exit_time and bar.close_price >= donchian_value_fast_up:
                # 入场开多
        ...
        if LLT_value_fast[-2] > LLT_value_slow[-2] and LLT_value_fast[-1] < LLT_value_slow[-1]:
            # 大致记录死叉点
            self.cross_down = (LLT_value_slow[-1] + LLT_value_slow[-2])/2
            # 如果突破
            if self.pos == 0 and bar.datetime.time() < self.exit_time and bar.close_price <= donchian_value_fast_down :
                # 入场开空
                if self.active_orderids:
                    self.write_log("撤单不干净，无法挂单")
                    return
        ...
        if self.pos > 0:
            ...
            self.stop_long = max(self.stop_long, self.cross_up, donchian_value_slow_down)
        ...
        if self.pos < 0:
            ...
            self.stop_short = min(self.stop_short, self.cross_down, donchian_value_slow_up)
