...
class TSMyoPPDCPROTOTYPEStrategy(CtaTemplate):
    """
    轴心点双通道股指日内
    """
    ...
    def __init__(self, cta_engine, strategy_name, vt_symbol, setting):
        """"""
        super(TSMyoPPDCPROTOTYPEStrategy, self).__init__(
    ...
    def on_bar(self, bar: BarData):
        ...
        # 30min唐安奇通道
        #self.tend_high, self.tend_low = am.donchian(30,False)
        ...
        if (bar.datetime.time() < self.exit_time):
            if self.pos == 0:
                ...
                if self.day_high > self.R1:
                    # 添加过滤，突破价不低于最高价，才进场
                    long_entry = max(self.R2, self.day_high)
                    short_entry = min(self.PP, self.day_low)            
                ...
                elif self.day_low < self.S1:
                    short_entry = min(self.S2, self.day_low)
                    long_entry = max(self.PP, self.day_high)
                    if self.vt_orderids or self.td_traded >= self.limited_size:
                        self.write_log("撤单不干净或达到交易限制，无法挂单")
                        return
                    orderids = self.short(short_entry, self.fixed_size, stop=True, lock=True)
