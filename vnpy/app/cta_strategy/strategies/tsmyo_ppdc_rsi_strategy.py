...
class TSMyoPPDCRSIStrategy(CtaTemplate):
    """
    轴心点双通道股指日内搭配RSI
    """
    ...
    parameters = ["trailing_stop", "fixed_size","limited_size","atr_stop","atr_window","rsi_filter","rsi_len","don_len"]
    variables = ["atr_value","rsi_value","PP","R1","R2","S1","S2","tend_high","tend_low"]

    def __init__(self, cta_engine, strategy_name, vt_symbol, setting):
        """"""
        super(TSMyoPPDCRSIStrategy, self).__init__(
    ...
    def on_bar(self, bar: BarData):
        ...
        self.rsi_value = am.rsi(self.rsi_len,False)
        # 30min唐安奇通道
        ...
        if (bar.datetime.time() < self.exit_time):
            # 设置RSI过滤
            if self.pos == 0 and abs(self.rsi_value-50)>self.rsi_filter:
