...
class TSMyoRBKDAYStrategy(CtaTemplate):
    """
    ...
    def __init__(self, cta_engine, strategy_name, vt_symbol, setting):
        """"""
        super(TSMyoRBKDAYStrategy, self).__init__(
    ...
    def on_bar(self, bar: BarData):
        ...
        if ( last_bar.datetime.date() != bar.datetime.date() and bar.datetime.time() > time(hour=8,minute=59)):
            if self.day_high:
        ...
        # 交易时间
        if (bar.datetime.time() < self.exit_time and bar.datetime.time() > time(hour=8,minute=59)):
            # 设置ATR过滤，只有波动扩大才开仓
